...
⭐UserMapper<T> 包含对单表各种增删改查方法

⭐mybatis plus操作的表是由实体类决定的
...
- **增加：Insert**
  ...
  ![image-20220906182439694](https://raw.githubusercontent.com/LifeSum12/typora-image/main/img/202209061836940.png)
...
mybatis plus中，在**执行插入数据前**会自动给id字段雪花算法生成一个bigint数据，无需自己输入。

  

- **删除：Delete**

  ![image-20220906182511086](https://raw.githubusercontent.com/LifeSum12/typora-image/main/img/202209061837963.png)

  4种：

  ​	根据id删除、

  ​	根据Map集合删除，delete from 表名 where x = ? and X = ? and ....

  ​	根据条件构造器删除

  ​	根据集合批量删除，delete from 表名 where id in(x,x,x,....)
...
- **修改：Update**
  ...
  ![image-20220906194325553](https://raw.githubusercontent.com/LifeSum12/typora-image/main/img/202209061943595.png)
...
- **查询：Selete**
  ...
  ![image-20220906194618819](https://raw.githubusercontent.com/LifeSum12/typora-image/main/img/202209061947389.png)
...
>
> 答：mybatis-plus的表和实体类，只能把id默认作为主键。uid、sid等都不好使。
...
排序条件：orderByDesc, orderByAsc

字段为空/不为空条件：isNull,  isNotNull
...
**updateWrapper和queryWrapper区别：**

​	对于查询来说，两者基本没啥区别，但是一般查询都是用`QueryWrapper`。

​	修改功能时，`QueryWrapper`只能先给查询条件，然后通过Mapper调用update。

​							`UpdateWrapper`有set操作，不用new对象，然后通过Mapper调用update。

updateWrapper举例：

<img src="https://raw.githubusercontent.com/LifeSum12/typora-image/main/img/202209121448482.png" alt="image-20220912144837379" style="zoom:67%;" />
...
⭐功能一：Mapper接口和对应Mapper映射文件**快速相互定位**
...
插件会自动在映射文件中生成语句。与baseMapper接口相比，mybatisX提供了更多的方法。

但方法名必须按要求定义才行。 insert、delete、update、select
...
通过数据库表，自动生成实体类、Mapper接口，Mapper映射文件。
